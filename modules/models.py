...
class BatchNormalization(tf.keras.layers.BatchNormalization):
    ...
    def __init__(self, axis=-1, momentum=0.9, epsilon=1e-5, center=True, scale=True, name=None, **kwargs):
        super(BatchNormalization, self).__init__(
            axis=axis, momentum=momentum, epsilon=epsilon, center=center, scale=scale, name=name, **kwargs
        )
...
def Backbone(backbone_type="ResNet50", use_pretrain=True):
    ...
    def backbone(x):
        if backbone_type == "ResNet50":
            extractor = ResNet50(input_shape=x.shape[1:], include_top=False, weights=weights)
            pick_layer1 = 80  # [80, 80, 512]
            pick_layer2 = 142  # [40, 40, 1024]
            pick_layer3 = 174  # [20, 20, 2048]
            preprocess = tf.keras.applications.resnet.preprocess_input
        elif backbone_type == "MobileNetV2":
            extractor = MobileNetV2(input_shape=x.shape[1:], include_top=False, weights=weights)
            pick_layer1 = 54  # [80, 80, 32]
            pick_layer2 = 116  # [40, 40, 96]
            pick_layer3 = 143  # [20, 20, 160]
            preprocess = tf.keras.applications.mobilenet_v2.preprocess_input
        else:
            raise NotImplementedError("Backbone type {} is not recognized.".format(backbone_type))
...
class ConvUnit(tf.keras.layers.Layer):
    ...
    def __init__(self, f, k, s, wd, act=None, name="ConvBN", **kwargs):
        ...
        if act is None:
            ...
            raise NotImplementedError("Activation function type {} is not recognized.".format(act))
