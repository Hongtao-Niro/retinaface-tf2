...
class ProgressBar(object):
    ...
    def update(self, inf_str=""):
        ...
        sys.stdout.write(stdout_str.format(bar_chars, self.completed, self.task_num, inf_str, self.fps))
...
#   Visulization                                                              #
###############################################################################
def draw_bbox_landm(img, ann, img_height, img_width, draw_score: bool = True, draw_lm: bool = True):
    """draw bboxes and landmarks"""
    # bbox
    x1, y1, x2, y2 = (
        int(ann[0] * img_width),
        int(ann[1] * img_height),
        int(ann[2] * img_width),
        int(ann[3] * img_height),
    )
    ...
    # landmark
    if ann[14] > 0 and draw_lm:
        cv2.circle(img, (int(ann[4] * img_width), int(ann[5] * img_height)), 1, (255, 255, 0), 2)
        cv2.circle(img, (int(ann[6] * img_width), int(ann[7] * img_height)), 1, (0, 255, 255), 2)
        cv2.circle(img, (int(ann[8] * img_width), int(ann[9] * img_height)), 1, (255, 0, 0), 2)
        cv2.circle(img, (int(ann[10] * img_width), int(ann[11] * img_height)), 1, (0, 100, 255), 2)
        cv2.circle(img, (int(ann[12] * img_width), int(ann[13] * img_height)), 1, (255, 0, 100), 2)
