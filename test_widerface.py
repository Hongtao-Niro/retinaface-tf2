...
flags.DEFINE_string(
    "save_folder", "./widerface_evaluate/widerface_txt/", "folder path to save evaluate results"
)
...
def main(_argv):
    ...
    for img_index, img_path in enumerate(img_paths):
        ...
        save_name = os.path.join(FLAGS.save_folder, sub_dir, img_name.replace(".jpg", ".txt"))

        pathlib.Path(os.path.join(FLAGS.save_folder, sub_dir)).mkdir(parents=True, exist_ok=True)
        ...
        # save images
        pathlib.Path(os.path.join("./results", cfg["sub_name"], sub_dir)).mkdir(parents=True, exist_ok=True)
        if FLAGS.save_image:
            for prior_index in range(len(outputs)):
                if outputs[prior_index][15] >= FLAGS.vis_th:
                    draw_bbox_landm(img_raw, outputs[prior_index], img_height_raw, img_width_raw)
            cv2.imwrite(os.path.join("./results", cfg["sub_name"], sub_dir, img_name), img_raw)
